...
CONFIG="$(realpath "${CONFIG:?set CONFIG=configs/rl/<x>.yaml}")"; NAME="$(basename "${CONFIG}" .yaml)"; NODES="${NODES:-4}"
...
if [[ "${SMOKE:-0}" == "1" ]]; then
  ...
  export COMMAND="cd /opt/nemo-rl && ${JUDGE_EXTRA}${BASECMD#cd /opt/nemo-rl && } cluster.num_nodes=${NODES}"
