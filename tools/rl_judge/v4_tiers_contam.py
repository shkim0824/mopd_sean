...
def tier(p):
    ...
    if p<=0.8: return '6/8'
    return '7-8/8'
print('=== sweep main tier by source x 4B tier (n, mean 30B pass, share 30B>=3/8) ===')
...
for b in ['aime24','aime25','aime26']:
    ...
    for l in open(f):
        ...
        for k in ['problem','question','prompt','input']:
            if k in o and isinstance(o[k],str): q=o[k]; break
        if q is None:
            for k,v in o.items():
                if isinstance(v,str) and len(v)>80 and 'boxed' not in v[:20]: q=v; break
